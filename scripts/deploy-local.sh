...
package_dir="${build_dir}/package/addons/xirepo"
default_addons_root="/Users/bferrari/Games/FFXI/HorizonXI/addons"
addons_root="${1:-${XIREPO_ADDONS_ROOT:-${default_addons_root}}}"
...
deploy_dir="${addons_root}/xirepo"
ashita_libs="${addons_root}/libs"

if [[ "$(basename "${addons_root}")" != "addons" || "$(basename "${deploy_dir}")" != "xirepo" ]]; then
...
if [[ -d "${ashita_libs}" ]]; then
    ...
    if [[ -f "${imgui_annotations}" ]]; then
        ...
        done < <(grep -Eo 'imgui\.[A-Za-z0-9_]+' "${repo_root}/addon/xirepo/versions/0.1.0/main.lua" | cut -d. -f2 | sort -u)
    fi
    ...
    done < <(grep -Eo 'ImGui[A-Za-z0-9_]+' "${repo_root}/addon/xirepo/versions/0.1.0/main.lua" | sort -u)
...
cmake --build "${build_dir}" --target xirepo_package

if [[ ! -f "${package_dir}/xirepo.lua" || ! -f "${package_dir}/versions/0.1.0/bin/xirepo_engine.dll" ]]; then
...
echo "Deployed xirepo to: ${deploy_dir}"
echo "If Ashita is running, unload xirepo before rebuilding and load it again afterward."
